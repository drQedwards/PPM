...
[ ] use forloopscode/contextplus or the fork drqedwards/contentplus repos as reference alongside this repo always
[ ] include PMLL 5 MCP TOOLS. use mcp/readme.md to find the specific tool names and use that information to update this agent_instructions.md so that PMLL's MCP SHORT term memory tools such as peek are included
[ ] compare test runtime speed using only Context+'s MCP tools using the original baseline of no tools at all to start. it is to be expected context+ runs faster than baseline (see contextplus's demo video and test demo for reference if needed) then run a new speed test but with the context+'s and PMLL's MCp tools and test if speed is even faster. 
[ ] read this article found in quotations as food thought: "Think of what I’m about to say as if we have three “agents”/“persons” that are about to go into committee to collaborate with their separate project contexts, but all three have the same role: software engineers. we directly compare the vectorized matricies of memory (the memory sheets) and overlay them and mark the differences in the context. There is also a separate short term cache of the semantic graph that works in parallel with long term ones. We will now discuss why we need both to maintain not only accuracy, but why this slaps better than a stateless group of Claudes who can never truly learn from each other, but also not learn the “wrong” things. 
